...
if [[ $UPDATE == "" ]] ; then
  ...
  iptables -A FORWARD -d 192.168.127.0 -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT &
  iptables -A FORWARD -s 192.168.127.0 -j ACCEPT &
  wait
  ...
  echo "now run: systemctl restart ocserv2"
fi
